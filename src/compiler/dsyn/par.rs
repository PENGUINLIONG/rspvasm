use super::mat::Match;
use crate::compiler::syn::{
    token::{Ident, Literal, Punct, Spacing},
    ParseBuffer,
};
...
impl ParseContext {
    ...
    pub fn peek(&mut self, par_ty: &ParType, input: &mut ParseBuffer) -> bool {
        match par_ty {
            ...
            ParType::Ident(ident) => input
                .clone()
                .parse::<Ident>()
                .map(|x| x.name == *ident)
                .unwrap_or(false),
            ParType::Punct(punct) => input
                .clone()
                .parse::<Punct>()
            ...
            ParType::Punctuated(par, _) => self.peek(&par.ty, input),
        }
    }
